...
const updateUser = async (req, res) => {
  ...
  if (!user)
    return res
      .status(400)
      .json({ message: `Employee coudln't be found with id ${req.body.id}` });
...
const deleteUser = async (req, res) => {
  if (!req?.body?.id)
    return res.status(400).json({ message: "Paas an userId to delete" });
...
const getAnUser = async (req, res) => {
  ...
  if (!req?.params?.id)
    return res.status(400).json({ message: "Pass an userId to get the user" });
